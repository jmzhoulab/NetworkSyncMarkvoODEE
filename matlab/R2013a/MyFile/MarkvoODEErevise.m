...
dt=0.01;        %迭代步长
t=0:dt:5;      %迭代时间序列
inititspan=round(10*(rand(nodenum,nodedim)-0.5));  
%inititspan=[3 1 -1;1 2 -5;1 -4 -1;-1 6 2;8 -3 1];        %各个节点状态的初始值
tanh=0;     %控制强度
randmatrix=round(rand(nodenum,nodenum));
tril=triu(randmatrix)-randmatrix;
L1=tril+tril'+diag(sum(tril+tril'));
randmatrix=round(rand(nodenum,nodenum));
tril=triu(randmatrix)-randmatrix;
L2=tril+tril'+diag(sum(tril+tril'));
randmatrix=round(rand(nodenum,nodenum));
tril=triu(randmatrix)-randmatrix;
L3=tril+tril'+diag(sum(tril+tril'));
D1=diag(round(rand(nodenum,1)));
D2=diag(round(rand(nodenum,1)));
D3=diag(round(rand(nodenum,1)));
% L1 =[2 -1 0 -1 0;
%     -1 2  -1 0 0;
%      0 -1  2 -1 0;
%     -1 0 -1 3 -1;
...
%      -1 0 -1 3 -1;...
%       0 -1 0 -1 2];
D1=diag([0 0 0 1 0]);D2=diag([1 0 0 0 0]);D3=diag([0 1 0 0 0]);     %三种不同的控制方式，如D1表示只控制节点1、2
